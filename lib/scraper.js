...
async function loginWithBrowser(email, password) {
  ...
  try {
    ...
    await browser.close();
    try { await redis.set(`data:v5:${email}`, JSON.stringify(data), 'EX', 3600); } catch(e) {}
...
async function scrapeAll(page) {
  ...
  // Dynamically compute planner page names for current + upcoming semester
  const _now = new Date(), _yr = _now.getFullYear(), _mo = _now.getMonth();
  const _planners = _mo >= 6
    ? [ // ODD semester (Jul-Dec): current ODD + next EVEN
        `Academic_Planner_${_yr}_${String(_yr+1).slice(-2)}_ODD`,
        `Academic_Planner_${_yr+1}_${String(_yr+2).slice(-2)}_EVEN`,
      ]
    : [ // EVEN semester (Jan-Jun): current EVEN + upcoming ODD
        `Academic_Planner_${_yr-1}_${String(_yr).slice(-2)}_EVEN`,
        `Academic_Planner_${_yr}_${String(_yr+1).slice(-2)}_ODD`,
      ];
...
async function startLogin(email, password, useSession, forceRefresh = false) {
  if (useSession) {
    ...
    if (!forceRefresh) {
      try {
        const cachedData = await redis.get(`data:v5:${email}`);
        if (cachedData) {
