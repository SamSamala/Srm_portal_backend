...
async function scrapeAll(cookies) {
  ...
  // Try the Zoho Creator JSON API for card/page views — returns JSON with record data
  async function fetchPlannerViaApi(name) {
    // Zoho Creator internal API: /view/{name} returns JSON with row data for authenticated sessions
    const apiUrl = `https://academia.srmist.edu.in/srm_university/academia-academic-services/view/${name}`;
    try {
      ...
      // JSON API returns {result: {rows: [...]}, ...} or similar
      if (text.trim().startsWith('{') || text.trim().startsWith('[')) {
        console.log('Planner API JSON response for', name, '— first 500:', text.substring(0, 500));
        try {
          const json = JSON.parse(text);
          return json; // caller will handle
        } catch(_) {}
      }
      // Not JSON — log a snippet of the HTML to find any API references
      const snippetPatterns = ['fetchUrl', 'apiUrl', 'ZCAJAX', 'zc_PageId', 'pagesId', '/view/', '/report/', 'startindex', 'pagesize'];
      for (const pat of snippetPatterns) {
        const idx = text.indexOf(pat);
        if (idx !== -1) {
          console.log(`[planner debug] Found '${pat}' in /view/ response:`, text.substring(Math.max(0, idx - 30), idx + 200));
        }
      }
    } catch(e) { console.warn('Planner API failed for', name, e.message); }
    return null;
  }

  async function fetchPlannerSem(sem) {
    for (const name of _plannerCandidates(sem)) {
      // 1. Try /report/ URL (list-view, parseable via pageSanitizer)
      ...
      // 2. Try Zoho Creator JSON API via /view/ endpoint
      const apiResult = await fetchPlannerViaApi(name);
      if (apiResult) {
        // If we get JSON data back, convert it to the HTML table format parsePlanner expects
        const rows = apiResult.result?.rows || apiResult.rows || (Array.isArray(apiResult) ? apiResult : null);
        if (rows && rows.length > 0) {
          console.log('Planner JSON API rows:', rows.length, '— first row keys:', Object.keys(rows[0]).join(','));
          // Build HTML table that parsePlanner can parse
          const header = Object.keys(rows[0]).join('\t');
          const body   = rows.map(r => Object.values(r).join('\t')).join('\n');
          return header + '\n' + body; // parsePlanner works on tab-separated text
        }
      }
    }
    // 3. Probe the /page/ URL to find AJAX endpoints in the JS
    const probeName = _plannerCandidates(sem)[0];
    try {
      const pr = await fetch(PAGE_URL_BASE + probeName, {
        headers: { 'Origin': 'https://academia.srmist.edu.in', 'User-Agent': UA, 'Cookie': cookieStr },
        redirect: 'follow',
      });
      const ph = await pr.text();
      const probePatterns = ['ZCAJAX', 'zc_PageId', 'pagesId', 'sectionId', 'fetchUrl', 'pageSection', 'card/fetch', 'view/', '/fetch', 'startindex', 'criterias'];
      for (const pat of probePatterns) {
        const idx = ph.toLowerCase().indexOf(pat.toLowerCase());
        if (idx !== -1) {
          console.log(`[planner probe '${pat}']:`, ph.substring(Math.max(0, idx - 20), idx + 250).replace(/\s+/g, ' '));
          break; // one snippet is enough per probe run
        }
      }
    } catch(_) {}
