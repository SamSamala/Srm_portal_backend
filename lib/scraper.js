...
function parseMarks(text) {
  ...
  for (const block of blocks) {
    ...
    if (!cm) continue;
    const tests = [];
    // Match any assessment pattern: 1-5 uppercase letters, dash, roman numeral
    // Covers FT-I, FJ-I, FP-I, FML-I, PBL-I, and any future formats
    const re = /([A-Z]{1,5}-[IVX]+)\/([ \d.]+)\s*\n?\s*([\d.]+|Abs)/g;
    let m;
  ...
  console.log(`[parseMarks] Found ${marks.length} subjects with marks`);
  return marks;
}
