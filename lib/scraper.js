...
async function fetchPage(cookies, viewLinkName) {
  ...
  if (startIdx === -1) {
    ...
    // Log a snippet around where the page name appears, to help debug name mismatches
    const nameIdx = html.indexOf(viewLinkName);
    if (nameIdx !== -1) {
      console.warn('Page name found in response but marker pattern missed. Snippet:', html.substring(nameIdx - 50, nameIdx + 100));
    }
    return null;
  }
...
async function scrapeAll(cookies) {
  ...
  async function fetchPlannerSem(sem) {
    for (const name of _plannerCandidates(sem)) {
      console.log('Trying planner page:', name);
      const html = await fetchPage(cookieStr, name);
