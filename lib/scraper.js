...
async function scrapeAll(cookies) {
  ...
  // One-time probe: dump page HTML script tags + first inline <script> block so we can find the AJAX URL
  async function probePlannerPageHTML(pageName) {
    try {
      ...
      console.log('[planner HTML probe] script srcs:', JSON.stringify(scriptSrcs.slice(0, 15)));
      // Dump the first inline <script> block that mentions pagesId or AJAX
      const blocks = [...html.matchAll(/<script(?![^>]+src)[^>]*>([\s\S]*?)<\/script>/g)];
      for (const b of blocks) {
        const content = b[1];
        if (/pagesId|ZCAJAX|zc_ajax|fetchUrl|initPage|loadPage|appPages|pageLoad/i.test(content)) {
          console.log('[planner HTML probe] inline script (first 2000):', content.substring(0, 2000).replace(/\s+/g, ' '));
          break;
        }
      }
      // Also dump 500 chars before+after pagesId in HTML
      const pi = html.indexOf('pagesId');
      if (pi !== -1) {
        console.log('[planner HTML probe] pagesId context:', html.substring(Math.max(0, pi - 300), pi + 500).replace(/\s+/g, ' '));
      }
