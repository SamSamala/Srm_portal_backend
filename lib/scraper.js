...
async function scrapeAll(cookies) {
  ...
  async function fetchPlannerSem(sem) {
    for (const name of _plannerCandidates(sem)) {
      ...
      // 2. Try direct base URL (no /page/ or /report/ prefix) — sometimes exposes list view
      const directUrl = 'https://academia.srmist.edu.in/srm_university/academia-academic-services/' + name;
      console.log('Trying planner direct URL:', directUrl);
      try {
        ...
        const m = `document.getElementById("zc-viewcontainer_${name}").innerHTML = pageSanitizer.sanitize('`;
        const si = h.indexOf(m);
        if (si !== -1) {
          const cs = si + m.length;
          let ce = -1;
          for (let i = cs; i < h.length - 2; i++) {
            if (h[i] === "'" && h[i+1] === ')' && h[i+2] === ';') {
              let bs = 0, j = i - 1;
              while (j >= 0 && h[j] === '\\') { bs++; j--; }
          ...
          if (ce !== -1) {
            console.log('Planner found via direct URL:', name);
            return jsUnescape(h.substring(cs, ce));
          }
        }
        // Log snippet to help diagnose
        const ni = h.indexOf(name);
        if (ni !== -1) console.log('Direct URL: page name found in response, snippet:', h.substring(ni - 30, ni + 120));
      } catch(e) { console.warn('Direct URL failed for', name, e.message); }
    }
