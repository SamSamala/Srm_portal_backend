...
async function scrapeAll(cookies) {
  ...
  // One-time probe: dump the first dscript widget content to understand calendar HTML structure
  async function probePlannerPageHTML(pageName) {
    try {
      const r = await fetch(PAGE_URL_BASE + pageName, {
        headers: { 'Origin': 'https://academia.srmist.edu.in', 'User-Agent': UA, 'Cookie': cookieStr },
        redirect: 'follow',
      });
      const html = await r.text();

      // Find the first dscript element and dump its content (this is the calendar widget HTML)
      const dscriptIdx = html.indexOf('eltype="dscript"');
      if (dscriptIdx !== -1) {
        // Walk back to the opening <div of this element
        let tagStart = dscriptIdx;
        while (tagStart > 0 && html[tagStart] !== '<') tagStart--;
        // Walk forward to the closing > of the opening tag
        const tagEnd = html.indexOf('>', dscriptIdx) + 1;
        // Extract 4000 chars of the element's content to see the HTML structure
        console.log('[planner dscript probe] first dscript element start:', html.substring(tagStart, tagEnd).replace(/\s+/g, ' '));
        console.log('[planner dscript probe] content (4000 chars):', html.substring(tagEnd, tagEnd + 4000).replace(/\s+/g, ' '));
      } else {
        // Dump the full elementsContainer content (3000 chars)
        const ecIdx = html.indexOf('<div id="elementsContainer"');
        if (ecIdx !== -1) {
          console.log('[planner container probe]:', html.substring(ecIdx, ecIdx + 3000).replace(/\s+/g, ' '));
        } else {
          // Dump first 3000 chars of body
          const bodyIdx = html.indexOf('<body');
          console.log('[planner body probe]:', html.substring(bodyIdx > 0 ? bodyIdx : 0, (bodyIdx > 0 ? bodyIdx : 0) + 3000).replace(/\s+/g, ' '));
        }
      }
    } catch(e) { console.warn('[planner HTML probe] failed:', e.message); }
  }
  ...
  async function fetchPlannerSem(sem) {
    ...
    // Last resort: dump page HTML to find the real AJAX endpoint (runs once)
    await probePlannerPageHTML(pageName);
